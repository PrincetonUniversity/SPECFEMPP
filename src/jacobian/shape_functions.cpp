...
type_real half_t = static_cast<type_real>(0.5);
} // namespace

std::vector<type_real> specfem::jacobian::define_shape_functions(
    const type_real xi, const type_real gamma, const int ngnod) {

  std::vector<type_real> h_shape2D(ngnod);
  ...
  if (ngnod == 4) {
    ...
    //----  midside nodes
    h_shape2D[4] = half_t * tm * gamma * (one - ss);
    h_shape2D[5] = half_t * sp * xi * (one - tt);
    h_shape2D[6] = half_t * tp * gamma * (one - ss);
    h_shape2D[7] = half_t * sm * xi * (one - tt);
...
specfem::jacobian::define_shape_functions_derivatives(const type_real xi,
                                                      const type_real gamma,
                                                      const int ngnod) {

  std::vector<std::vector<type_real> > h_dershape2D(
      ndim, std::vector<type_real>(ngnod));
  if (ngnod == 4) {
    const type_real sp = xi + one;
    const type_real sm = xi - one;
    const type_real tp = gamma + one;
    ...
    h_dershape2D[0][5] = half_t * (one - tt) * (s2 + one);
    h_dershape2D[0][6] = -one * st * tp;
    h_dershape2D[0][7] = half_t * (one - tt) * (s2 - one);

    h_dershape2D[1][4] = half_t * (one - ss) * (t2 - one);
    h_dershape2D[1][5] = -one * st * sp;
    h_dershape2D[1][6] = half_t * (one - ss) * (t2 + one);
...
void specfem::jacobian::define_shape_functions(HostView1d shape2D, type_real xi,
  ...
  if (ngnod == 4) {
    ...
    //----  midside nodes
    shape2D(4) = half_t * tm * gamma * (one - ss);
    shape2D(5) = half_t * sp * xi * (one - tt);
    shape2D(6) = half_t * tp * gamma * (one - ss);
    shape2D(7) = half_t * sm * xi * (one - tt);
...
void specfem::jacobian::define_shape_functions_derivatives(
  ...
  if (ngnod == 4) {
    ...
    dershape2D(0, 5) = half_t * (one - tt) * (s2 + one);
    dershape2D(0, 6) = -one * st * tp;
    dershape2D(0, 7) = half_t * (one - tt) * (s2 - one);

    dershape2D(1, 4) = half_t * (one - ss) * (t2 - one);
    dershape2D(1, 5) = -one * st * sp;
    dershape2D(1, 6) = half_t * (one - ss) * (t2 + one);
