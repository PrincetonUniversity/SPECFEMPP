...
specfem::adjacency_graph::adjacency_graph<4> from_index_mapping(
      const Kokkos::View<int ***, Kokkos::LayoutLeft, Kokkos::DefaultExecutionSpace> &
      index_mapping){
  ...
  if(ngllz < 3 || ngllx < 3){
    throw std::runtime_error("specfem::adjacency_graph::from_index_mapping() relies on >= 3 quadrature points per axis");
  }
  ...
  std::vector<specfem::adjacency_graph::adjacency_pointer> inv_index(nglob,specfem::adjacency_graph::adjacency_pointer());

  const auto set_ref_inds = [&](int& ix, int& iz, const int8_t side, const bool flip, const int off = 1) {
  ...
  const auto verify_adjacencies = [&](const int ispec, const int side) {
    ...
    if(!inv_index[iglob].is_active()){
      ...
      inv_index[iglob] = specfem::adjacency_graph::adjacency_pointer(ispec,side,false);
      ...
      if(iglob_corner_L == iglob_corner_L_ && iglob_corner_H == iglob_corner_H_){
        //no flip
        graph.form_adjacency(ispec,side,ispec_,side_,false);
      }else if(iglob_corner_L == iglob_corner_H_ && iglob_corner_H == iglob_corner_L_){
        //flip
        graph.form_adjacency(ispec,side,ispec_,side_,true);
      }else{
        throw std::runtime_error("specfem::adjacency_graph::from_index_mapping() - error encountered during index assignment.");
      }
    }
  };
...
static bool _reflect(const specfem::adjacency_graph::adjacency_graph<4> & graph,
    int& ispec_set, int& ix_set, int& iz_set, const int& ispec, const int& ix, const int& iz, const int8_t& side,
    const int& ngllz, const int& ngllx){
  specfem::adjacency_graph::adjacency_pointer adj = graph.get_adjacency(ispec,side);
  ispec_set = adj.elem;
...
static void _propagate_index(const specfem::adjacency_graph::adjacency_graph<4> & graph,
    Kokkos::View<int ***, Kokkos::LayoutLeft, Kokkos::DefaultExecutionSpace>& index_mapping,
  ...
  if(ix == ngllx-1 && _reflect(graph,ispec_,ix_,iz_,ispec,ix,iz,0,ngllz,ngllx) && index_mapping(ispec_,iz_,ix_) == -1){
    _propagate_index(graph,index_mapping,ispec_,ix_,iz_,index,ngllz,ngllx);
  }else if(ix == 0 && _reflect(graph,ispec_,ix_,iz_,ispec,ix,iz,2,ngllz,ngllx) && index_mapping(ispec_,iz_,ix_) == -1){
    _propagate_index(graph,index_mapping,ispec_,ix_,iz_,index,ngllz,ngllx);
  }else if(iz == ngllz-1 && _reflect(graph,ispec_,ix_,iz_,ispec,ix,iz,1,ngllz,ngllx) && index_mapping(ispec_,iz_,ix_) == -1){
    _propagate_index(graph,index_mapping,ispec_,ix_,iz_,index,ngllz,ngllx);
  }if(iz == 0 && _reflect(graph,ispec_,ix_,iz_,ispec,ix,iz,3,ngllz,ngllx) && index_mapping(ispec_,iz_,ix_) == -1){
    _propagate_index(graph,index_mapping,ispec_,ix_,iz_,index,ngllz,ngllx);
  }
}

Kokkos::View<int ***, Kokkos::LayoutLeft, Kokkos::DefaultExecutionSpace>
        to_index_mapping(const specfem::adjacency_graph::adjacency_graph<4> & graph, const int ngllz, const int ngllx, int* nglob_set){
  
  int nspec = graph.get_size();
  Kokkos::View<int ***, Kokkos::LayoutLeft, Kokkos::DefaultExecutionSpace> index_mapping("specfem::adjacency_graph::to_index_mapping()",nspec,ngllz,ngllx);
    ...
    for(int iz = 0; iz < ngllz; iz++){
      for(int ispec = 0; ispec < nspec; ispec++){
        if(index_mapping(ispec,iz,ix) == -1){
          ...
          _propagate_index(graph,index_mapping,ispec,ix,iz,count,ngllz,ngllx);
          count++;
        }
      }
    }
