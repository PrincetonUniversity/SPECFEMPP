...
std::string IO::param_file::param_read(std::string name,
  ...
  if (pos != std::string::npos) {
    std::string name = name.substr(pos);
    ;
  }
...
void IO::param_file::read(type_real &value, std::string name,
  ...
  if (string_read == "")
    throw std::runtime_error("Missing parameter " + name);
  value = std::atof(string_read.c_str());
}
...
void IO::read_parameters_file(std::string parameters_file,
                              specfem::parameters &param) {

  IO::param_file param_file(parameters_file);
  param_file.open();
  ...
  try {
    ...
    try {
      param_file.read(param.partitioning_type, "partitioning_method");
      std::cout
  ...
  */
  ...
  try {
    param_file.read(param.n_sls, "NSLS");
  } catch (std::runtime_error &e) {
    std::cerr << e.what() << std::endl;
    some_parameters_are_missing = true;
  }
  ...
  try {
    ...
    try {
      param_file.read(param.attenuation_f0_reference,
                      "ATTENUATION_f0_REFERENCE");
    } catch (std::runtime_error &e) {
      std::cerr << e.what() << std::endl;
      some_parameters_are_missing = true;
  ...
  */
  ...
  try {
    param_file.read(param.PML_boundary_conditions, "PML_BOUNDARY_CONDITIONS");
  } catch (std::runtime_error &e) {
    std::cerr << e.what() << std::endl;
    some_parameters_are_missing = true;
  }
  ...
  try {
    param_file.read(param.stacey_absorbing_boundary_conditions,
...
void IO::read_sources_file(std::string source_file,
  ...
  IO::param_file param_file(source_file);
  param_file.open();
  bool some_parameters_are_missing = false;
