...
//                            GET_NAME(MEDIUM_TAG));                              \
//   if (properties.h_element_types(lcoord.ispec) == GET_TAG(MEDIUM_TAG)) {       \
//     CREATE_VARIABLE_NAME(source, GET_NAME(DIMENSION_TAG),                      \
//                          GET_NAME(MEDIUM_TAG))                                 \
//         .push_back(source);                                                    \
//     h_source_domain_index_mapping(lcoord.ispec) =                              \
//         CREATE_VARIABLE_NAME(source, GET_NAME(DIMENSION_TAG),                  \
//                              GET_NAME(MEDIUM_TAG))                             \
//             .size() -                                                          \
//         1;                                                                     \
...
//   }
  ...
  //     CALL_MACRO_FOR_ALL_MEDIUM_TAGS(SORT_SOURCES_PER_MEDIUM,
  ...
  // #define COUNT_SOURCES(DIMENSION_TAG, MEDIUM_TAG)                               \
//   sources += CREATE_VARIABLE_NAME(source, GET_NAME(DIMENSION_TAG),             \
//                                   GET_NAME(MEDIUM_TAG))                        \
//                  .size();
  ...
  //   if (nsources != sources.size()) {
  //     throw std::runtime_error(
  //         "Not all sources were assigned or sources are assigned multiple
  //         times");
  //   }

  //   Kokkos::deep_copy(source_domain_index_mapping,
  //   h_source_domain_index_mapping); Kokkos::deep_copy(medium_types,
  //   h_medium_types); Kokkos::deep_copy(wavefield_types, h_wavefield_types);

  // #define CREATE_SOURCE_MEDIUM(DIMENSION_TAG, MEDIUM_TAG)                        \
//   CREATE_VARIABLE_NAME(source, GET_NAME(DIMENSION_TAG),                        \
//                        GET_NAME(MEDIUM_TAG)) =                                 \
//       specfem::compute::impl::source_medium<GET_TAG(DIMENSION_TAG),            \
//                                             GET_TAG(MEDIUM_TAG)>(              \
//           CREATE_VARIABLE_NAME(source, GET_NAME(DIMENSION_TAG),                \
//                                GET_NAME(MEDIUM_TAG)),                          \
//           mesh, partial_derivatives, properties, t0, dt, nsteps);

  //   CALL_MACRO_FOR_ALL_MEDIUM_TAGS(
  //       CREATE_SOURCE_MEDIUM,
  //       WHERE(DIMENSION_TAG_DIM2) WHERE(MEDIUM_TAG_ELASTIC,
  //       MEDIUM_TAG_ACOUSTIC))

  // #undef CREATE_SOURCE_MEDIUM
}
...
//       h_source_domain_index_mapping(lcoord.ispec) = acoustic_sources.size()
//       - 1; h_medium_types(lcoord.ispec) = acoustic;
//       h_wavefield_types(lcoord.ispec) = source->get_wavefield_type();
//     } else if (properties.h_element_types(lcoord.ispec) == elastic) {
//       elastic_sources.push_back(source);
//       h_source_domain_index_mapping(lcoord.ispec) = elastic_sources.size()
//       - 1; h_medium_types(lcoord.ispec) = elastic;
//       h_wavefield_types(lcoord.ispec) = source->get_wavefield_type();
//     } else {
...
//       specfem::element::medium_tag::acoustic>( acoustic_sources, mesh,
//       partial_derivatives, properties, t0, dt, nsteps);

//   this->elastic_sources = specfem::compute::impl::source_medium<
//       specfem::dimension::type::dim2,
//       specfem::element::medium_tag::elastic>( elastic_sources, mesh,
//       partial_derivatives, properties, t0, dt, nsteps);
// }
