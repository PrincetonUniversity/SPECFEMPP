...
def should_flip(edge1: int, edge2: int):
    """Returns whether or not the along-edge coordinate is flipped when edge1
and edge2 are conformally joined.
    """
...
for edge1, estr1 in enumerate(edges):
    ...
    for edge2, estr2 in enumerate(edges):
        ...
        vary_coord = "ngll - 1 - iedge" if should_flip(edge1, edge2) else
"iedge" internal += spacing * 2 + f"case {macro_EDGEIND_OF(estr2, 4)}:\n"
