...
void construct_demo_mesh(specfem::mesh::mesh & mesh, const specfem::quadrature::quadratures & quad,
  ...
  Kokkos::View<long double ****, Kokkos::HostSpace> pts("temp_point_storage",mesh.nspec,ngllz,ngllx,NDIM);
  for(int ielemz = 0; ielemz < nelemz; ielemz++){
  ...
  specfem::material::material<specfem::element::medium_tag::acoustic,
          specfem::element::property_tag::isotropic> acoustic_holder(
      density, cp, Qkappa, Qmu, compaction_grad);

  mesh.materials = specfem::mesh::materials();
  mesh.materials.n_materials = 1;
  mesh.materials.material_index_mapping = 
      specfem::kokkos::HostView1d<specfem::mesh::materials::material_specification>(
        "specfem::mesh::material_index_mapping", mesh.nspec); //will be populated with matspec on populate loop
  specfem::mesh::materials::material_specification matspec(specfem::element::medium_tag::acoustic,
            specfem::element::property_tag::isotropic, 0);
  
  std::vector<specfem::material::material<specfem::element::medium_tag::elastic, specfem::element::property_tag::isotropic> >
      l_elastic_isotropic(0);
  std::vector<specfem::material::material<specfem::element::medium_tag::acoustic, specfem::element::property_tag::isotropic> >
  ...
  mesh.materials.elastic_isotropic = specfem::mesh::materials::material<
        specfem::element::medium_tag::elastic, specfem::element::property_tag::isotropic>(
      l_elastic_isotropic.size(), l_elastic_isotropic);
  mesh.materials.acoustic_isotropic = specfem::mesh::materials::material<
        specfem::element::medium_tag::acoustic, specfem::element::property_tag::isotropic>(
  ...
  mesh.control_nodes = specfem::mesh::control_nodes(NDIM, mesh.nspec, ngnod, mesh.npgeo);
  ...
  for(int ielemz = 0; ielemz < nelemz; ielemz++){
    for(int ielemx = 0; ielemx < nelemx; ielemx++){
      ...
      removals.push_back(specfem::adjacency_graph::adjacency_pointer(ispec,specfem::enums::boundaries::type::BOTTOM));
      removals.push_back(specfem::adjacency_graph::adjacency_pointer(ispec,specfem::enums::boundaries::type::TOP));
      ...
      for (int icomp = 0; icomp < NDIM; icomp++){
        mesh.control_nodes.coord(icomp,off + 0) = (type_real) pts(ispec,0,0,icomp);
      }
      mesh.control_nodes.knods(1,ispec) = off + 1;
      for (int icomp = 0; icomp < NDIM; icomp++){
        mesh.control_nodes.coord(icomp,off + 1) = (type_real) pts(ispec,0,ngllx-1,icomp);
      }
      mesh.control_nodes.knods(2,ispec) = off + 2;
      for (int icomp = 0; icomp < NDIM; icomp++){
        mesh.control_nodes.coord(icomp,off + 2) = (type_real) pts(ispec,ngllz-1,ngllx-1,icomp);
      }
      mesh.control_nodes.knods(3,ispec) = off + 3;
      for (int icomp = 0; icomp < NDIM; icomp++){
        mesh.control_nodes.coord(icomp,off + 3) = (type_real) pts(ispec,ngllz-1,0,icomp);
      }
    }
  }
...
void construct_demo_mesh(specfem::mesh::mesh & mesh, const specfem::quadrature::quadratures & quad,
  ...
  std::vector<specfem::adjacency_graph::adjacency_pointer> removals;
  _util::demo_assembly::construct_demo_mesh(mesh, quad, nelemx, nelemz, removals);
}
void construct_demo_mesh(specfem::mesh::mesh & mesh, const specfem::quadrature::quadratures & quad){
  std::vector<specfem::adjacency_graph::adjacency_pointer> removals;
