...
void benchmark(specfem::compute::assembly &assembly,
  ...
  for (const auto [istep, dt] : time_scheme->iterate_forward()) {
    ...
    // compute_stiffness_interaction2<acoustic, isotropic, flag>(assembly, istep);
    // compute_stiffness_interaction2<elastic, isotropic, flag>(assembly, istep);
    ...
    if constexpr (flag) {
      ...
      compute_stiffness_interaction2<elastic, anisotropic, false>(assembly, istep);
    }
