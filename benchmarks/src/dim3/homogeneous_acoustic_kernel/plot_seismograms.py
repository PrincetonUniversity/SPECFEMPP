...
def read_seismogram(filename):
    ...
    return data[:, 0], data[:, 1]  # time, pressure
...
def main():
    ...
    # Read MXP pressure seismograms
    seismogram_files = sorted(glob.glob("OUTPUT_FILES/results/*.S3.MXP.semp"))
    reference_files = sorted(glob.glob("reference_seismograms/*.S3.MXP.semp"))
    ...
    for filename in seismogram_files:
        ...
        time, pressure = read_seismogram(filename)
        all_seismograms[station_name] = (time, pressure)
        time_range = (min(time_range[0], time.min()), max(time_range[1], time.max()))
        max_pressure = max(max_pressure, np.abs(pressure).max())

    for filename in reference_files:
        station_name = filename.split("/")[-1].split(".")[1]
        time, pressure = read_seismogram(filename)
        reference_seismograms[station_name] = (time, pressure)
        time_range = (min(time_range[0], time.min()), max(time_range[1], time.max()))
        max_pressure = max(max_pressure, np.abs(pressure).max())

    # Subplot 2: Pressure seismograms
    ax2 = fig.add_subplot(gs[0, 1])
    y_spacing = max_pressure * 2.5 if max_pressure > 0 else 1.0
    ...
    for j, station in enumerate(stations_sorted):
        ...
        if station_name in all_seismograms:
            time, pressure = all_seismograms[station_name]
            normalized = (
                pressure / max_pressure * y_spacing * 0.8
                if max_pressure > 0
                else pressure
            )
            ax2.plot(
        ...
        if station_name in reference_seismograms:
            time_ref, pressure_ref = reference_seismograms[station_name]
            normalized_ref = (
                pressure_ref / max_pressure * y_spacing * 0.8
                if max_pressure > 0
                else pressure_ref
            )
            ax2.plot(
    ...
    ax2.set_title("Component MXP (Pressure)")
