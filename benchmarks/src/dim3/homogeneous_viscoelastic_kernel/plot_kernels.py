"""
Plot elastic sensitivity kernels on the source-receiver plane (X-Z at Y=center).
"""
...
def load_kernels(kernels_dir):
    subdir = os.path.join(kernels_dir, "Kernels", "elastic_isotropic")
    if not os.path.isdir(subdir):
        kernels_root = os.path.join(kernels_dir, "Kernels")
        proc_dirs = sorted(
            os.path.join(kernels_root, entry, "elastic_isotropic")
            for entry in os.listdir(kernels_root)
            if entry.startswith("proc_")
            and os.path.isdir(os.path.join(kernels_root, entry, "elastic_isotropic"))
        )
        if not proc_dirs:
            raise FileNotFoundError(f"No elastic kernel files found in {kernels_root}")

        arrays = {}
        kernel_names = ("rho", "mu", "kappa", "rhop", "alpha", "beta")
        ...
        return (
            ...
            arrays["Y"],
            arrays["Z"],
            {name: arrays[name] for name in kernel_names},
        )
    ...
    kernel_names = ("rho", "mu", "kappa", "rhop", "alpha", "beta")
    kernels = {
        name: np.load(os.path.join(subdir, f"{name}.npy")).flatten()
        for name in kernel_names
    }
    return X, Y, Z, kernels
...
def main():
    ...
    for ax, (name, label) in zip(axes.ravel(), labels.items()):
        plot_kernel_slice(ax, x, z, ks[name], label, source, station)
        ax.set_title(label)

    fig.suptitle(f"Elastic kernels — X–Z plane at Y = {y_center / 1000:.0f} km", y=1.01)
    plt.tight_layout()
