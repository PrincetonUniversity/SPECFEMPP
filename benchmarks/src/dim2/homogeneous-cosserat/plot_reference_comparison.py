...
def main():
    ...
    station_indices = {name: idx for idx, name in enumerate(sorted_stations)}
    ...
    fig, axes = plt.subplots(n_stations, 3, figsize=(12, 0.7*n_stations), 
    ...
    for station_idx in range(n_stations):
        ...
        axes[station_idx, 0].plot(ux_ref_norm, 'k-', linewidth=0.8, 
                                 label='Reference' if station_idx==0 else "", clip_on=False)
        axes[station_idx, 0].plot(ux_spec_norm, 'r--', linewidth=0.8, alpha=0.8,
                                 label='SPECFEM' if station_idx==0 else "", clip_on=False)
        ...
        axes[station_idx, 1].plot(uz_spec_norm, 'r--', linewidth=0.8, alpha=0.8, clip_on=False)
        ...
        axes[station_idx, 2].plot(ry_spec_norm, 'r--', linewidth=0.8, alpha=0.8, clip_on=False)
        
        # Add legend to first row
        if station_idx == 0:
            axes[station_idx, 0].legend(frameon=False, bbox_to_anchor=(1.0, 1.0), loc='lower right')
        ...
        for col in range(3):
            ...
            if station_idx == 0:
                ...
                axes[station_idx, col].text(0.02, 0.98, f'{scale_val:.2e}', 
