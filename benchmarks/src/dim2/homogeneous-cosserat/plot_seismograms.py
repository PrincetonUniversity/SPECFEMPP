...
def main():
    ...
    with open("OUTPUT_FILES/STATIONS", "r") as f:
        for line in f:
            if line.strip():
                ...
                stations_info[station_name] = {'network': network, 'x': x_coord, 'z': z_coord}
    ...
    def get_station_distance(station_name):
        if station_name in stations_info:
            ...
            distance = np.sqrt((station_x - source_x)**2 + (station_z - source_z)**2)
            return distance
        return float('inf')
    ...
    def sort_traces_by_station_order(stream, component):
        ...
        return [traces_dict[station] for station in sorted_stations if station in traces_dict]
    ...
    fig, axes = plt.subplots(n_stations, 4, figsize=(16, 0.7*n_stations), 
    ...
    for station_idx in range(n_stations):
        ...
        if station_idx < len(traces_dx):
            ...
            axes[station_idx, 0].plot(time, normalized_data, 'k-', linewidth=0.8, clip_on=False)
        ...
        if station_idx < len(traces_dz):
            ...
            axes[station_idx, 1].plot(time, normalized_data, 'k-', linewidth=0.8, clip_on=False)
        ...
        if station_idx < len(traces_r) and station_idx < len(traces_c):
            ...
            axes[station_idx, 2].plot(time_r, normalized_r, 'k-', linewidth=0.8, label='Rotation' if station_idx==0 else "", clip_on=False)
            axes[station_idx, 2].plot(time_c, normalized_c, 'r--', linewidth=1.0, alpha=0.8, label="$0.5\\nabla \\times \\mathbf{s}$" if station_idx==0 else "", clip_on=False)
            if station_idx == 0:
                axes[station_idx, 2].legend(frameon=False, bbox_to_anchor=(1.0, 1.0), loc='lower right')
        ...
        if station_idx < len(traces_ir):
            ...
            axes[station_idx, 3].plot(time, normalized_data, 'k-', linewidth=0.8, label='Intrinsic Rotation' if station_idx==0 else "", clip_on=False)
            if station_idx == 0:
                axes[station_idx, 3].legend(frameon=False, bbox_to_anchor=(1.0, 1.0), loc='lower right')
        ...
        for col in range(4):
            axes[station_idx, col].grid(True, alpha=1.0, linestyle='-', linewidth=0.5, axis='x', zorder=-1)
            ...
            if station_idx == 0:
                ymax = col_ymaxes[col]
                axes[station_idx, col].text(0.02, 0.98, f'{ymax:.2e}', 
    ...
    for station_idx in range(n_stations):  # All except last row
        if station_idx < n_stations - 1:
            for col in range(4):
                axes[station_idx, col].tick_params(axis='x', labelbottom=False, bottom=False)
        else:
            for col in range(4):
                axes[station_idx, col].tick_params(axis='x', labelbottom=True, bottom=False)
