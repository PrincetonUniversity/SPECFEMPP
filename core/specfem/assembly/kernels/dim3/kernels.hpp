...
/**
 * @brief 3D spectral element misfit kernels container for seismic inversion
 *
 * This template specialization provides comprehensive storage and management
 * for all types of misfit kernels in 3D spectral element simulations.
 *
 * **Physical Context:**
 ...
 *
 * @code
 * // Initialize kernels container for 3D mesh
 * specfem::assembly::kernels<specfem::element::dimension_tag::dim3> kernels(
 *     nspec, ngllz, nglly, ngllx, element_types);
 *
 ...
 */
template <>
struct kernels<specfem::element::dimension_tag::dim3>
...
public:
   ...
   *
   * Initializes the complete kernels storage system for a 3D spectral element
   ...
   *
   * @param nspec Total number of spectral elements in the 3D mesh
   * @param ngllz Number of Gauss-Lobatto-Legendre quadrature points in
   * z-direction
   * @param nglly Number of Gauss-Lobatto-Legendre quadrature points in
   * y-direction
