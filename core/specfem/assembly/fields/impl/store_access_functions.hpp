...
KOKKOS_FORCEINLINE_FUNCTION void store_after_simd_dispatch(
  ...
  for (int icomp = 0; icomp < ncomponents; ++icomp) {
    (base_store_accessor<on_device, DataClass>(iglob, icomp, field,
                                               accessors(icomp)),
     ...);
  }
  return;
...
KOKKOS_FORCEINLINE_FUNCTION void store_after_simd_dispatch(
  ...
  // Call load for each accessor
  for (int icomp = 0; icomp < ncomponents; ++icomp) {
    (base_store_accessor<on_device, DataClass>(iglob, icomp, mask, TagType(),
...
template <bool on_device, typename IndexType, typename ContainerType,
          ...
          typename std::enable_if_t<
              ...
              int> = 0>
KOKKOS_FORCEINLINE_FUNCTION void
store_after_field_access(const IndexType &index, const ContainerType &field,
  ...
  // Call load for each accessor
  store_after_simd_dispatch<on_device>(simd_accessor_type(), index, field,
                                       accessors...);
  return;
}
