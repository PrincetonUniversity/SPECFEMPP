...
struct attenuation_medium<specfem::element::dimension_tag::dim2,
  ...
  // shape [nspec_attn][ngllz][ngllx]. Element-constant today (broadcast to every
  // GLL point) but stored per-GLL for forward-compat with GLL-varying Q.
  // Recomputed from Q + frequency band; used by model I/O to (un)scale kappa/mu.
  ...
  // Host-only per-GLL quality factors: shape [nspec_attn][ngllz][ngllx].
  ...
  attenuation_medium(
    ...
    h_kappa_scale = scalar_view_type::host_mirror_type("h_kappa_scale",
                                                       nspec_attn, ngllz, ngllx);
    ...
    h_Qkappa =
        scalar_view_type::host_mirror_type("h_Qkappa", nspec_attn, ngllz, ngllx);
    h_Qmu =
    ...
    for (int i = 0; i < nspec_attn; ++i) {
      ...
      // Store the (element-constant) quality factors and modulus scale factors
      // at every GLL point for model I/O. Q is not used at runtime; the scale
      // factors let the writer/reader (un)scale kappa/mu between the physical
      // (relaxed) and runtime (unrelaxed) representations.
      const type_real Qkappa = material.Qkappa;
      const type_real Qmu = material.Qmu;
      for (int iz = 0; iz < ngllz; ++iz) {
        for (int ix = 0; ix < ngllx; ++ix) {
          h_Qkappa(i, iz, ix) = Qkappa;
          h_Qmu(i, iz, ix) = Qmu;
   ...
   * (matching how the mesh path derives it). Host-only; mirrors the scale
   * computation in @ref ComputedAttenuationValues. Q is sampled per GLL point,
   * so a GLL-varying Q model is honoured. The (expensive, Nelder-Mead)
   * tau_epsilon solve is memoized and only redone when Q changes from the
   * previous point, so element-constant Q costs one solve per element.
   */
        ...
        for (int ix = 0; ix < l_ngllx; ++ix) {
          ...
          if (Qmu != last_Qmu) {
            mu_scale =
                specfem::attenuation::get_attenuation_scale_factor<N_SLS>(
                    fc_.raw(),
                    specfem::attenuation::compute_tau_eps<N_SLS>(Qmu,
                                                                 tau_sigma_,
                                                                 band_),
  ...
  void recompute_relaxation_rates(const KappaView &h_kappa, const MuView &h_mu,
    ...
    for (std::size_t gi = 0; gi < h_kappa.extent(0); ++gi) {
      ...
      for (std::size_t iz = 0; iz < h_kappa.extent(1); ++iz)
        for (std::size_t ix = 0; ix < h_kappa.extent(2); ++ix) {
          const type_real Qkappa = h_Qkappa(a, iz, ix);
          const type_real Qmu = h_Qmu(a, iz, ix);
          if (Qkappa != last_Qkappa) {
            kappa_props =
          ...
          const type_real kappa_sc = h_kappa(gi, iz, ix);
          const type_real mu_sc = h_mu(gi, iz, ix);
          for (int j = 0; j < N_SLS; ++j) {
            const type_real tauinv_j = 1.0 / tau_sigma_(j);
            h_kappa_relaxation_rate(a, iz, ix, j) =
                kappa_sc * kappa_props.beta(j) * tauinv_j /
                kappa_props.one_minus_sum_beta;
            h_mu_relaxation_rate(a, iz, ix, j) =
