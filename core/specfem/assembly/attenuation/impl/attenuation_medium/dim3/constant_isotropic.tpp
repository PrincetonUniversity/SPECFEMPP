...
struct attenuation_medium<specfem::element::dimension_tag::dim3,
  ...
  attenuation_medium(
    ...
    if (allocate_io_views) {
      h_kappa_ref = scalar_view_type::host_mirror_type(
          "h_kappa_ref", nspec_attn, ngllz, nglly, ngllx);
        ...
        for (int iz = 0; iz < ngllz; ++iz) {
          for (int iy = 0; iy < nglly; ++iy) {
            for (int ix = 0; ix < ngllx; ++ix) {
              ...
              h_kappa_ref(i, iz, iy, ix) = material.kappa_reference();
              h_mu_ref(i, iz, iy, ix) = material.mu_reference();
            }
          }
        }
      }
  ...
  void recompute(
    ...
    const int l_nglly = h_kappa_relaxation_rate.extent(2);
    const int l_ngllx = h_kappa_relaxation_rate.extent(3);
    for (int i = 0; i < nspec_attn; ++i) {
        ...
        for (int iy = 0; iy < l_nglly; ++iy) {
          for (int ix = 0; ix < l_ngllx; ++ix) {
            ...
            if (Qkappa != last_Qkappa) {
              const auto tau_eps =
                  specfem::attenuation::compute_tau_eps<N_SLS>(Qkappa,
                                                               tau_sigma, band);
              kappa_scale =
            ...
            const type_real kappa_unrelaxed =
                h_kappa_ref(i, iz, iy, ix) * kappa_scale;
            const type_real mu_unrelaxed = h_mu_ref(i, iz, iy, ix) * mu_scale;
