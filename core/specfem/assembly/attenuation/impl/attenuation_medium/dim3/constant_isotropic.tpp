...
struct attenuation_medium<specfem::element::dimension_tag::dim3,
  ...
  // shape [nspec_attn][ngllz][nglly][ngllx]. Element-constant at construction
  // but stored per GLL so a GLL-varying Q model read from disk is honoured.
          ...
          for (int ix = 0; ix < l_ngllx; ++ix) {
            ...
            for (int j = 0; j < N_SLS; ++j) {
              ...
              h_kappa_relaxation_rate(i, iz, iy, ix, j) =
                  h_kappa(gi, iz, iy, ix) * kappa_props.beta(j) * tauinv_j /
                  kappa_props.one_minus_sum_beta;
              h_mu_relaxation_rate(i, iz, iy, ix, j) =
                  h_mu(gi, iz, iy, ix) * 2.0 * mu_props.beta(j) * tauinv_j /
                  mu_props.one_minus_sum_beta;
            }
          }
        }
      ...
      for (std::size_t iz = 0; iz < view.extent(1); ++iz)
        for (std::size_t iy = 0; iy < view.extent(2); ++iy)
          for (std::size_t ix = 0; ix < view.extent(3); ++ix) {
            const type_real s = attenuating ? scale(a, iz, iy, ix)
                                            : static_cast<type_real>(1);
            scratch(gi, iz, iy, ix) = view(gi, iz, iy, ix) / s;
          }
    }
    return scratch;
  }
  ...
  void scale_to_runtime(const ViewType &view, const ScaleViewType &scale,
    ...
    for (int gi = 0; gi < static_cast<int>(element_indices.size()); ++gi) {
      ...
      for (std::size_t iz = 0; iz < view.extent(1); ++iz)
        for (std::size_t iy = 0; iy < view.extent(2); ++iy)
          for (std::size_t ix = 0; ix < view.extent(3); ++ix)
            view(gi, iz, iy, ix) *= scale(a, iz, iy, ix);
    }
  }
