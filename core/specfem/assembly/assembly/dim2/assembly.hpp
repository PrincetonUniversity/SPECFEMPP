...
template <> struct assembly<specfem::element::dimension_tag::dim2> {
  ...
  /**
   * @brief Element intersections for every intersection (edge) on coupled
   * interface in the mesh.
   *
   * The intersection type defines the flux scheme to use when computing
   * coupling terms between two media (e.g., fluid-solid interface).
   *
   */
