 ...
 * @brief Material property container for 2D acoustic media at spectral element
 * quadrature points.
 *
 * This data container stores the essential material properties required for
 * acoustic wave propagation simulation in 2D domains. Acoustic media are
 * characterized by a single wave mode (pressure waves) governed by the acoustic
 * wave equation:
 *
 * \f$
 ...
 * \f$
 *
 * where \f$p\f$ is pressure, \f$\mathbf{v}\f$ is velocity, \f$\rho\f$ is
 * density, and \f$\kappa\f$ is bulk modulus.
 *
 * **Performance Optimization:**
 * The container stores inverse quantities (\f$1/\rho\f$, \f$1/\kappa\f$) that
 * are frequently used in time integration schemes, eliminating expensive
 * division operations during the computation-intensive assembly phase.
 *
 * **Derived Properties:**
 * Additional acoustic properties can be computed from the stored values:
 * - Acoustic wave speed: \f$c = \sqrt{\kappa/\rho}\f$
 * - Acoustic impedance: \f$Z = \rho c = \sqrt{\rho \kappa}\f$
 * - Quality factor: \f$Q\f$ (for attenuating media)
 *
 * @tparam UseSIMD Boolean indicating whether to use SIMD intrinsics for
 * vectorized operations across multiple quadrature points simultaneously. When
 * true, enables processing multiple properties in parallel using CPU vector
 * instructions.
 *
 * **Generated Accessor Functions:**
 *
 * The following accessor functions are automatically generated by the
 * POINT_CONTAINER macro:
 *
 * - **Inverse density**: \f$\frac{1}{\rho}\f$ - Used in velocity updates during
 * time integration
 *   @code KOKKOS_INLINE_FUNCTION const value_type rho_inverse() const @endcode
 *
 * - **Bulk modulus**: \f$\kappa\f$ - Relates pressure and volumetric
 * deformation
 *   @code KOKKOS_INLINE_FUNCTION const value_type kappa() const @endcode
 *
 * **Additional Computed Properties:**
 *
 * - **Inverse bulk modulus**: \f$\frac{1}{\kappa}\f$ - Used in pressure updates
 *   @code KOKKOS_INLINE_FUNCTION const value_type kappa_inverse() const
 * @endcode
 *
 * - **Inverse acoustic impedance**: \f$\frac{1}{\rho v_p}\f$ - Used in
 * absorbing boundary conditions
 *   @code KOKKOS_INLINE_FUNCTION const value_type rho_vpinverse() const
 * @endcode
 *
 * @note The rho_vpinverse() function is particularly important for implementing
 *       Stacey absorbing boundary conditions in acoustic media.
 *
 * @see specfem::boundary_conditions::acoustic_stacey
 * @see specfem::time_integration::acoustic
 *
 * @code
 * // Example: Setting up acoustic properties for water
 * using AcousticProps = specfem::point::properties::data_container<
 *     specfem::dimension::type::dim2,
 *     specfem::element::medium_tag::acoustic,
 *     specfem::element::property_tag::isotropic, false>;
 *
 * // Water properties
 * const type_real density = 1000.0;     // kg/m³
 * const type_real bulk_modulus = 2.2e9; // Pa
 *
 * AcousticProps props(1.0/density, bulk_modulus);
 *
 * // Access properties
 * auto rho_inv = props.rho_inverse();     // 1/1000
 * auto kappa = props.kappa();             // 2.2e9
 * auto kappa_inv = props.kappa_inverse(); // 1/(2.2e9)
 *
 * // Compute derived quantities
 * auto wave_speed = std::sqrt(kappa * rho_inv);
 * @endcode
 */
