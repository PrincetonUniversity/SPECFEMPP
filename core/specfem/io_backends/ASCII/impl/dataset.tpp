...
specfem::io_backends::impl::ASCII::Dataset<ViewType, OpType>::Dataset(
    boost::filesystem::path &folder_name, const std::string &name,
    const ViewType data)
    : data(data), DatasetBase<OpType>(folder_name, name, rank,
                                      [&data]() -> std::vector<int> {
...
template <typename ViewType, typename OpType>
void specfem::io_backends::impl::ASCII::Dataset<ViewType, OpType>::write() {
...
template <typename ViewType, typename OpType>
void specfem::io_backends::impl::ASCII::Dataset<ViewType, OpType>::read() {
  if (std::is_same_v<MemSpace, Kokkos::HostSpace>) {
