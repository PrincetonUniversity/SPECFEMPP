...
public:
  ...
  specfem::io_backends::impl::HDF5::Dataset<ViewType, OpType>
  createDataset(const std::string &name, const ViewType data) {
    throw std::runtime_error("SPECFEM++ was not compiled with HDF5 support");
  }

  specfem::io_backends::impl::HDF5::Group<OpType>
...
public:
   ...
   * @return specfem::io_backends::impl::HDF5::Dataset<ViewType, OpType> Dataset
   * object
   */
  template <typename ViewType>
  specfem::io_backends::impl::HDF5::Dataset<ViewType, OpType>
  createDataset(const std::string &name, const ViewType data) {
    return specfem::io_backends::impl::HDF5::Dataset<ViewType, OpType>(
        group, name, data);
  }
   ...
   *
   * @param name Name of the group
   * @return specfem::io_backends::impl::HDF5::Group<OpType> Group object
   */
  specfem::io_backends::impl::HDF5::Group<OpType>
  createGroup(const std::string &name) {
    return specfem::io_backends::impl::HDF5::Group<OpType>(group, name);
  }
