...
constexpr auto ellipsoid =
    specfem::ellipticity::ellipsoid<specfem::constants::default_ellipsoid>{};
...
specfem::coordinate_systems::transform<
  ...
  // Kept this one since it would make the code unwieldy for the reader.
  ...
  const double f1 = (1.0 - utm_impl::e2 / 4.0 - 3.0 * utm_impl::e4 / 64.0 -
                     5.0 * utm_impl::e6 / 256.0) *
                    rlat;
  const double f2 = (3.0 * utm_impl::e2 / 8.0 + 3.0 * utm_impl::e4 / 32.0 +
                     45.0 * utm_impl::e6 / 1024.0) *
                    std::sin(2.0 * rlat);
  const double f3 =
      (15.0 * utm_impl::e4 / 256.0 + 45.0 * utm_impl::e6 / 1024.0) *
      std::sin(4.0 * rlat);
  const double f4 = (35.0 * utm_impl::e6 / 3072.0) * std::sin(6.0 * rlat);
  const double rm = utm_impl::semi_major_axis * (f1 - f2 + f3 - f4);
  ...
  if (dlat == 90.0 || dlat == -90.0) {
    // Special case: poles
    xx = 0.0;
    yy = utm_impl::utm_scaling_factor * rm;
  } else {
    // Radius of curvature in prime vertical (Snyder eq. 4-20)
    const double sin_rlat = std::sin(rlat);
    const double cos_rlat = std::cos(rlat);
    const double rn = utm_impl::semi_major_axis /
                      std::sqrt(1.0 - utm_impl::e2 * sin_rlat * sin_rlat);

    // Snyder eq. 8-13, 8-14
    const double t = std::tan(rlat) * std::tan(rlat);
    const double c = utm_impl::ep2 * cos_rlat * cos_rlat;
    ...
    const double x_f2 =
        (5.0 - 18.0 * t + t * t + 72.0 * c - 58.0 * utm_impl::ep2) * a5 / 120.0;
    xx = utm_impl::utm_scaling_factor * rn * (a + x_f1 + x_f2);

    // Snyder eq. 8-10 for northing (y)
    const double y_f1 = a2 / 2.0;
    const double y_f2 = (5.0 - t + 9.0 * c + 4.0 * c * c) * a4 / 24.0;
    const double y_f3 =
        (61.0 - 58.0 * t + t * t + 600.0 * c - 330.0 * utm_impl::ep2) * a6 /
        720.0;
    yy = utm_impl::utm_scaling_factor *
         (rm + rn * std::tan(rlat) * (y_f1 + y_f2 + y_f3));
  }

  xx = xx + utm_impl::false_easting;
  yy = yy + utm_impl::false_northing;
...
specfem::coordinate_systems::transform<
  ...
  double yy = cart.y - utm_impl::false_northing;
  if (lsouth)
    yy = yy - 1.0e7;

  // Snyder eq. 3-24 for e1
  const double sqrt_1_minus_e2 = std::sqrt(1.0 - utm_impl::e2);
  const double e1 = (1.0 - sqrt_1_minus_e2) / (1.0 + sqrt_1_minus_e2);

  // Snyder eq. 7-19 for footpoint latitude (mu)
  const double rm = yy / utm_impl::utm_scaling_factor;
  const double mu_denom = 1.0 - utm_impl::e2 / 4.0 - 3.0 * utm_impl::e4 / 64.0 -
                          5.0 * utm_impl::e6 / 256.0;
  const double mu = rm / (utm_impl::semi_major_axis * mu_denom);
  ...
  if (dlat1 >= 90.0 || dlat1 <= -90.0) {
    ...
    const double c1 = utm_impl::ep2 * cos_rlat1 * cos_rlat1;
    const double t1 = tan_rlat1 * tan_rlat1;

    const double f1_denom = 1.0 - utm_impl::e2 * sin_rlat1 * sin_rlat1;
    // Radius of curvature in prime vertical (Snyder eq. 4-20)
    const double rn1 = utm_impl::semi_major_axis / std::sqrt(f1_denom);
    // Radius of curvature in meridional plane
    const double r1 = utm_impl::semi_major_axis * (1.0 - utm_impl::e2) /
                      std::sqrt(f1_denom * f1_denom * f1_denom);
    const double d = xx / (rn1 * utm_impl::utm_scaling_factor);
    ...
    const double lat_f3 =
        (5.0 + 3.0 * t1 + 10.0 * c1 - 4.0 * c1 * c1 - 9.0 * utm_impl::ep2) *
        d4 / 24.0;
