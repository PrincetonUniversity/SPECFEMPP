...
void specfem::linear_system::compute_element_stiffness(
  ...
  if (impl == specfem::linear_system::StiffnessKernelImpl::sum_factored) {
#ifdef SPECFEM_ENABLE_TENSOROPS
    specfem::linear_system_impl::compute_element_stiffness_sum_factored<NGLL,
                                                                        Tags>(
        assembly, batch, k_e);
    return;
#else
    throw std::runtime_error(
        "specfem::linear_system::compute_element_stiffness: the sum_factored "
