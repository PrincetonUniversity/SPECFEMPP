...
/**
 * @defgroup
 * specfem_medium_dim3_compute_source_contribution_elastic_isotropic_cosserat
 *
 */
 ...
 * @ingroup
 * specfem_medium_dim3_compute_source_contribution_elastic_isotropic_cosserat
 * @brief Compute source contribution for 3D elastic isotropic Cosserat media.
 *
 ...
 * - \f$ \ddot{\phi}_x = S^c_x(t) \cdot L_x(\mathbf{x}) \f$
 * - \f$ \ddot{\phi}_y = S^c_y(t) \cdot L_y(\mathbf{x}) \f$
 * - \f$ \ddot{\phi}_z = S^c_z(t) \cdot L_z(\mathbf{x}) \f$
 *
 ...
 * @return Acceleration contributions [\f$\ddot{u}_x, \ddot{u}_y, \ddot{u}_z,
 * \ddot{\phi}_x, \ddot{\phi}_y, \ddot{\phi}_z\f$]
 */
...
KOKKOS_INLINE_FUNCTION auto impl_compute_source_contribution(
  ...
  result(4) = point_source.stf(4) * point_source.lagrange_interpolant(4);
  result(5) = point_source.stf(5) * point_source.lagrange_interpolant(5);
  return result;
}
