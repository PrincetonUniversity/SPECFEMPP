...
namespace specfem::chunk_face::impl {

/**
 * @brief Accessor-agnostic field implementation for chunk-based face data.
 *
 * This class provides a accessor-agnostic implementation for chunk-based
 * face data, primarily used when making unified references to different
 ...
 *
 * @tparam ChunkSize Number of faces processed together in a chunk
 * @tparam NGLL Number of Gauss-Lobatto-Legendre points per spatial dimension
 * @tparam DimensionTag Spatial dimension (dim3) of the field
...
public:
  ...
  /// @brief Number of faces in the chunk
  constexpr static int nfaces = ChunkSize;
...
public:
   ...
   * implementation, but typically follows the pattern: (face_id,
   * gll_i, gll_j, component_id).
   *
...
/**
 * @brief Chunk face field accessor for storing field values at all
 *        quadrature points within a chunk.
 *
 * This class provides an accessor interface for chunk-based face field data,
 * supporting efficient storage and access to field values at all
 * Gauss-Lobatto-Legendre (GLL) quadrature points within a chunk of faces.
 * It is designed for use in chunk execution policies where spatial locality of
 * data is critical for performance. Faces are 2D surfaces in 3D, so the
 * storage is organized as [nfaces][ngll][ngll][components].
 *
 * @tparam ChunkSize     Number of faces processed together in a chunk.
...
public:
  ...
  using value_type = typename base_type::template vector_type<type_real, nfaces,
                                                              ngll, components>;
...
public:
   ...
   *
   * Provides access to individual field values using multi-dimensional
   * indexing. For chunk face fields, the indexing typically follows the
   * pattern: (face_id, gll_i, gll_j, component_id).
   *
   * @tparam Indices Parameter pack for multi-dimensional indices
   * @param indices The indices specifying the location and component to access
   * @return Reference to the field value at the specified location
   *
   * @code{.cpp}
   * // For 3D face fields: (face, i_gll, j_gll, component)
   * auto disp_x = field(iface, i, j, 0);  // x-component
   * auto disp_y = field(iface, i, j, 1);  // y-component
   * auto disp_z = field(iface, i, j, 2);  // z-component
   * field(iface, i, j, 0) = new_value;
   * @endcode
   */
  template <typename... Indices>
  KOKKOS_FORCEINLINE_FUNCTION typename value_type::value_type &
