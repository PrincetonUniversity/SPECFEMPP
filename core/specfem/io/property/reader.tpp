        ...
        if constexpr (AttenuationType::template has_attenuation<
          ...
          const auto &att =
              assembly.attenuation.template get_container<medium_tag,
                                                          property_tag>();
          // Read the attenuation model datasets (e.g. Qkappa/Qmu), then
          // recompute the runtime attenuation state from them (scale factors,
          // unrelaxed moduli, relaxation rates). Both are no-ops when there
          // are no attenuating elements.
          att.for_each_io_host_view(
              [&](const auto view, const std::string view_name) {
                group.openDataset(view_name, view).read();
              });
          att.recompute(container, element_indices, assembly.attenuation.fc,
