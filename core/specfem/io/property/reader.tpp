...
void specfem::io::property_reader<InputLibrary>::read(
  ...
  specfem::tag_dispatch::for_each(
      ...
      [&]<typename TagsType>() {
        ...
        // Property datasets are serialized in logical row-major order (see
        // sub_block.hpp), so every read is staged through a plain scratch
        // view and unpacked into the chunk-tiled container view. "kappa" and
        // "mu" land as the physical (relaxed) moduli from the file.
        container.for_each_host_view(
            [&](const auto view, const std::string view_name) {
              ...
              specfem::io::property_impl::insert_sub_block(view, scratch,
                                                           offset);
            });
        ...
        if constexpr (has_attenuation) {
          const auto &att =
              assembly.attenuation.template get_container<medium_tag,
                                                          property_tag>();
          // Attenuation model datasets (Qkappa/Qmu): plain host views, read
          // verbatim. recompute then rescales the just-read physical moduli
          // in the property container to their runtime (unrelaxed) values
          // and rebuilds the relaxation rates.
          for (const auto &[view_name, view] : att.get_views()) {
