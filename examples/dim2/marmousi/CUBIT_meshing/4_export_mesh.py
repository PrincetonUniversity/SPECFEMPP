...
#
# creates block entities to export mesh with the export_cubit.py script
#
...
# (not necessary anymore as export_cubit.py script will recognize in which plane the model lies)
if 1 == 0:
...
if 1 == 0:
    ...
    cubit.cmd(
        "block " + str(id_block) + " element type QUAD4"
    )  # convert if elements types are SHELL4 to QUAD4

    # elastic material
    if 1 == 1:
        print("# using elastic material definition")
        cubit.cmd(
            "block " + str(id_block) + ' name "elastic 1" '
        )  # elastic material region
        cubit.cmd("block " + str(id_block) + " attribute count 6")
        cubit.cmd(
            "block " + str(id_block) + " attribute index 1 1"
    ...
    if 1 == 0:
        ...
        cubit.cmd(
            "block " + str(id_block) + ' name "acoustic 1" '
        )  # acoustic material region
        ...
        cubit.cmd(
            "block " + str(id_block) + " attribute index 4 1028 "
        )  # rho (ocean salt water density:
...
if 1 == 1:
    ...
    for i in surfaces:
        ...
        cubit.cmd(
            "block " + str(id_block) + ' name "elastic ' + str(isurf) + '" '
        )  # elastic material region
        cubit.cmd("block " + str(id_block) + " attribute count 6")
        cubit.cmd(
            "block " + str(id_block) + " attribute index 1 " + str(isurf)
        )  # flag for material: 1 for 1. material, 2 for 2. material, ..
        cubit.cmd("block " + str(id_block) + " attribute index 2 2800")  # vp
        cubit.cmd("block " + str(id_block) + " attribute index 3 1500")  # vs
        cubit.cmd("block " + str(id_block) + " attribute index 4 2300")  # rho
...
script_directory = os.path.join(this_files_directory, "../../../../scripts")
...
try:
    ...
    print(
        "could not import from `../../../../scripts/export_cubit` , please check your working dir and PYTHONPATH settings..."
    )
    print("")
    print("current path: ")
    print(sys.path)
    print("")
    print(
        "try to include path to directory which includes file boundary_definition.py and export_cubit.py, e.g. SPECFEM2D/utils/export_cubit/"
    )
    print("")
...
try:
    ...
    print("Error: Importing script export_cubit failed")
    print(
        "could not import from `../../../../scripts/export_cubit` , please check your working dir and PYTHONPATH settings..."
    )
    print("")
    ...
    print("")
    print(
        "try to include path to directory which includes file boundary_definition.py and export_cubit.py, e.g. SPECFEMPP/scripts/"
    )
    print("")
    sys.exit("Import export_cubit failed")
