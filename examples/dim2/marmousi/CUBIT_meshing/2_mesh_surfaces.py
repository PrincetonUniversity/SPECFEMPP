...
# original
cubit.cmd(
    "surface 186  size " + str(elem_size_min)
)  # otherwise a meshing error occurs..
...
if 0 == 1:
    # large curve
    cubit.cmd("curve 227 to 229 size " + str(elem_size))  # noqa: F821
    cubit.cmd("curve 227 to 229 scheme equal")
    cubit.cmd("mesh curve 227 to 229")
    # progressive sizes
    cubit.cmd(
        "curve 230 scheme bias fine size 0.2 coarse size "
        + str(elem_size)  # noqa: F821
        + " start vertex 290"
    )
    ...
    # progressive sizes
    cubit.cmd(
        "curve 581 scheme bias fine size 0.2 coarse size "
        + str(elem_size)  # noqa: F821
        + " start vertex 291"
    )
...
def mesh_surface(i, elem_size_min, elem_size_max):
    ...
    if 0 == 1:
        ...
        #
        cubit.silent_cmd(
            'group "poor" add quality surface '
            + str(i)
            + " shape low "
            + str(shape_min)
            + " high "
        ...
        while len(quads) > 0 and itry < 8:
            ...
            if elem_size1 > elem_size2:
                ...
                cubit.cmd(
                    "surface "
                    + str(i)
                    + " sizing function type skeleton min_size "
                    + str(elem_size1)
                    + " max_size "
                    + str(elem_size2)
                    + " max_gradient 2.0 min_num_layers_2d 1 min_num_layers_1d 1"
                )
            cubit.cmd("mesh surface " + str(i))
            # check mesh
            cubit.silent_cmd(
