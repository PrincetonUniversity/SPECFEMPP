...
def convert_to_velocity():
    ...
    for i in range(435):
        ...
        if rock_type == 1:
            ...
            # limestone
            print(
                "Error limestone not defined in Table 2.8, please check data line:",
                iline,
                " -- ",
                txt,
            )
        ...
        if vs < 0.0:
            print(
                "Error invalid shear velocity ", txt, " --- vp/vs/rho = ", vp, vs, rho
            )
            sys.exit(1)
        ...
        if iline == 240:
            if use_solid_domain_for_water_layer:
                ...
                print(
                    "  replacing water layer with solid velocities (from Capdeville et al. 2010)"
                )
                print("")
                pass
            else:
                ...
                k = 0.0
                rho = 1.01
                rho = 1000.0 * rho  # converts g/cm^3 to kg/m^3
        ...
        if iline == 1:
            ...
            if use_solid_domain_for_water_layer:
                f_out.write(
                    "#   replacing water layer with solid velocities (material_id==240)\n"
                )
            else:
        ...
        # entry
        f_out.write(
            "%d %3d %10.4f %10.4f %10.4f 9999.0 9999.0 %f\n"
    ...
    print("")
    print(
        "now, consider replacing the default `MESH/nummaterial_velocity_file` with this marmousi2 file,"
    )
    print(
        "or in the `Par_file` change the filename entry `nummaterial_velocity_file = ..` accordingly."
    )
    print("")
    print("all done")
    print("")
...
def usage():
    print("usage:")
    print(
        "    ./convert_surface_rock_to_velocities.py [--with-water / --without-water]"
    )
    print("  with")
    print(
        "    --with-water             - uses top water layer from original model (default)"
    )
    print(
        "    --without-water          - replaces top water layer from original model with solid velocities (as in Capdeville et al. 2010)"
    )
    sys.exit(1)
