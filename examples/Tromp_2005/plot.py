...
def plot_data(ax, X, Z, data, title, cmap):
    ...
    # bar levels
    cbar = plt.colorbar(ax.contourf(X, Z, data, cmap = cmap, levels=1000, vmin = -1.5e-8, vmax = 1.5e-8), ax = ax)
    ax.set_title(title)
    ax.set_xlabel('X (km)')
    ax.set_ylabel('Z (km)')
...
def plot_kernels(input_directory, output):
    ...
    # Preprocess the data
    X_grid, Z_grid, data = preprocess_data(X, Z, rho = rho, kappa = kappa, mu = mu, rhop = rhop, alpha = alpha, beta = beta)
