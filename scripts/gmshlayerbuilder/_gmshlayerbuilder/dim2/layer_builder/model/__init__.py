...
from .edges import ConformingInterfaces, EdgeType, edges_of_all_elements
...
class Model:
    ...
    def from_meshed_surface(surface: list[int] | int, gmsh: GmshContext) -> "Model":
        ...
        if isinstance(surface, int):
            ...
            # nonconform = NonconformingInterfaces()

            # for blayer in built_layerbds[1:-1]:
            #     nonconform.concatenate(
            #         NonconformingInterfaces.between_curves(
            #             blayer,
            #             remap_node_indices=node_indexing.apply,
            #             node_locs=node_locs,
