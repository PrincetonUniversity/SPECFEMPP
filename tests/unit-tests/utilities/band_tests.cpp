...
TEST(Utilities_Band, BandSecondsToHertz) {
  constexpr type_real min_p = 1.0, max_p = 100.0;
  Band<Seconds> bp{ Seconds(min_p), Seconds(max_p) };
...
TEST(Utilities_Band, BandSecondsToOmega) {
...
TEST(Utilities_Band, FromPeriodRoundTrip) {
  constexpr type_real min_p = 1.0, max_p = 100.0;
  Band<Seconds> b{ Seconds(min_p), Seconds(max_p) };
...
TEST(Utilities_Band, FromFrequencyRoundTrip) {
  constexpr type_real min_f = 0.01, max_f = 1.0;
  Band<Hertz> b{ Hertz(min_f), Hertz(max_f) };
...
TEST(Utilities_Band, FromOmegaRoundTrip) {
  constexpr type_real min_w = 0.5, max_w = 50.0;
  Band<Omega> b{ Omega(min_w), Omega(max_w) };
...
TEST(Utilities_Band, RepresentationsAreEquivalent) {
...
TEST(Utilities_Band, PeriodFrequencyConsistency) {
...
TEST(Utilities_Band, OmegaOrdering) {
  Band<Seconds> bp{ Seconds(1.0), Seconds(100.0) };
  auto bw = unit_cast<Omega>(bp);
