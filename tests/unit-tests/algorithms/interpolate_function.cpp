...
TEST(ALGORITHMS, interpolate_function) {
  ...
  // Assemble
  specfem::assembly::mesh<specfem::dimension::type::dim2> assembly_mesh(
      mesh.tags, mesh.control_nodes, quadratures, mesh.adjacency_graph);

  const auto xi = assembly_mesh.h_xi;
  const auto gamma = assembly_mesh.h_xi;
  ...
  for (int iz = 0; iz < N; ++iz) {
    for (int ix = 0; ix < N; ++ix) {
      ...
      auto gcoord = specfem::algorithms::locate_point(lcoord, assembly_mesh);
