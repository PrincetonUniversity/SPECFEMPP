...
private:
  //==== for handling interdependent vars (3 DoF, 4 vars). the lower prior
  // values define the DoF.
  // higher values get overwritten.
  int8_t _t0_adj_prio;
  int8_t _dt_adj_prio;
  ...
  void _update_timevars(int set_ind) {
    /* set_ind
      ...
      set_prio_prev is the priority of that var
    */
    ...
    // All prio vals less than set_prio_prev need to shift ++ to take up the
    // previous prio,
    //  since set_ind's prio goes to 0
    //  while managing that logic, update the value that has prio == 3
    //  (overwrite)
  ...
  bool overwrite_nseismo_steps; // by default, nseismo is nsteps
  int _nseismogram_steps;
