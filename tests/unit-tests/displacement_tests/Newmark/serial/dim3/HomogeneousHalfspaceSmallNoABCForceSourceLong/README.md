...
This is the same test as `HomogeneousHalfspaceSmallNoABCForceSource` but with a longer simulation time. This test is used to check whether the free surface boundary condition is correctly implemented, or rather, no boundary condition is implemented since SEM elastic should have nothing applied.
