...
TEST_F(PointCoordinatesTest, LocalCoordinates2D_InsideOutside) {
  using local2d =
      specfem::point::local_coordinates<specfem::element::dimension_tag::dim2>;
  const type_real bound = 1.001;

  // Strictly inside
  local2d in(3, 0.5, -0.3);
  EXPECT_TRUE(in.inside());
  EXPECT_FALSE(in.outside(bound));

  // On the boundary (|coord| == 1)
  local2d edge(3, 1.0, -1.0);
  EXPECT_TRUE(edge.inside());
  EXPECT_FALSE(edge.outside(bound));

  // In the tolerance band (1 < |coord| < bound): neither inside nor outside
  local2d band(3, 1.0005, 0.0);
  EXPECT_FALSE(band.inside());
  EXPECT_FALSE(band.outside(bound));

  // Beyond the tolerance
  local2d out(3, 1.05, 0.0);
  EXPECT_FALSE(out.inside());
  EXPECT_TRUE(out.outside(bound));

  // Unlocated point (ispec < 0): not inside; counts as outside even with
  // in-range coordinates.
  local2d unlocated(-1, 0.5, -0.3);
  EXPECT_FALSE(unlocated.inside());
  EXPECT_TRUE(unlocated.outside(bound));
}

// 3D reference-element predicates.
TEST_F(PointCoordinatesTest, LocalCoordinates3D_InsideOutside) {
  using local3d =
      specfem::point::local_coordinates<specfem::element::dimension_tag::dim3>;
  const type_real bound = 1.001;
  ...
  // In the tolerance band (1 < |eta| < bound): neither inside nor outside
