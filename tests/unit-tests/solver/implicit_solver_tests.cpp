...
TEST(ImplicitSolver3D, ConstructsAndOperatorMatchesOnStaceyMesh) {
  ...
  VectorType x(layout.owned_map());
  x.randomize();

  VectorType a_x(layout.owned_map()), reference(layout.owned_map()),
      scratch(layout.owned_map());
  ...
  type_real max_diff = 0;
  {
    ...
    for (std::size_t dof = 0;
         dof < static_cast<std::size_t>(layout.num_global_dofs()); ++dof) {
