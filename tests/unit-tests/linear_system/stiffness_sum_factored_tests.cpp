...
// A/B test holding the sum-factored kernel to the probe kernel: same inputs,
// same K_e contract, agreement to roundoff. The probe is the oracle -- it is
// correct by construction (it applies the production matrix-free operator),
// while the sum-factored kernel re-derives the same weak form in closed form.
namespace stiffness_sum_factored_test {
...
class SumFactoredStiffness3D : public ::testing::Test {
protected:
...
AssemblyType *SumFactoredStiffness3D::assembly_ = nullptr;

TEST_F(SumFactoredStiffness3D, AgreesWithProbeKernelWithTiming) {
  const auto range = elastic_range();
  const int nelements = range.size();

  StiffnessView k_probe("k_probe", nelements, ndof, ndof);
  StiffnessView k_sum_factored("k_sum_factored", nelements, ndof, ndof);

  // Warm-up runs first (view allocations, first-touch); the timed pair after.
  fill_blocks(k_probe, KernelImpl::probe, range);
  fill_blocks(k_sum_factored, KernelImpl::sum_factored, range);
  const double probe_ms = fill_blocks(k_probe, KernelImpl::probe, range);
  const double sum_factored_ms =
      fill_blocks(k_sum_factored, KernelImpl::sum_factored, range);

  std::printf("[ timing   ] probe: %.2f ms, sum_factored: %.2f ms "
              "(%d elements, speedup %.1fx)\n",
              probe_ms, sum_factored_ms, nelements, probe_ms / sum_factored_ms);

  auto h_probe =
      Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace{}, k_probe);
  auto h_sum =
      Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace{}, k_sum_factored);
  ...
  for (int e = 0; e < nelements; ++e) {
    for (int i = 0; i < ndof; ++i) {
      for (int j = 0; j < ndof; ++j) {
        const type_real diff = std::abs(h_probe(e, i, j) - h_sum(e, i, j));
        if (diff > max_diff) {
          max_diff = diff;
          worst_e = e;
          worst_i = i;
  ...
  // The two kernels evaluate the same bilinear form through different
  // operation orders, so they agree only to roundoff of the largest entry.
  ...
  EXPECT_LE(max_diff, tol) << "worst entry at (e=" << worst_e
                           ...
                           << " sum_factored="
                           << h_sum(worst_e, worst_i, worst_j)
                           << " scale=" << scale;
}

TEST_F(SumFactoredStiffness3D, SymmetricWithRigidBodyNullSpace) {
  const auto range = elastic_range();
  const specfem::datatype::ElementIndexRange first(range.begin_index(),
                                                   range.begin_index() + 1);
  StiffnessView k_e("k_e", 1, ndof, ndof);
  specfem::linear_system::compute_element_stiffness<StiffnessTags>(
      *assembly_, first, k_e, KernelImpl::sum_factored);
  ...
  // K is symmetric for elastic isotropic media; the sum-factored kernel
  // computes K(i, j) and K(j, i) from the symmetric G tensor through
  // differently-ordered products, so they match only up to roundoff.
...
TEST(SumFactoredStiffness3D, SkippedWithoutTensorOps) {
  GTEST_SKIP() << "SPECFEM++ was built without TensorOperations "
                  "(SPECFEM_ENABLE_TENSOROPS=OFF); the sum-factored stiffness "
