...
#include "specfem/attenuation/setup.hpp"
...
public:
  ...
  bool has_source = false; ///< whether a reference source solution was given
};
...
public:
  ...
  config(const YAML::Node &Node) {
    ...
    // Build the attenuation Setup passed to read_2d_mesh. The frequency band
    // comes from the Setup (not the mesh database), so attenuating configs
    // must declare it.
    attenuation_setup.enabled = attenuation_enabled;
    if (attenuation_enabled) {
      if (!Node["attenuation_f0"].IsDefined() ||
          !Node["attenuation_band"].IsDefined()) {
        throw std::runtime_error(
            "attenuation_enabled requires attenuation_f0 and "
            "attenuation_band in the test configuration");
      }
      attenuation_setup.f0 =
          specfem::units::quantity_cast<specfem::units::Hertz>(
              Node["attenuation_f0"].as<std::string>());
      const YAML::Node band = Node["attenuation_band"];
      attenuation_setup.band = specfem::utilities::Band<specfem::units::Hertz>(
          specfem::units::quantity_cast<specfem::units::Hertz>(
...
private:
  ...
  specfem::attenuation::Setup attenuation_setup;
};
...
public:
  Test(const YAML::Node &Node) {
    ...
    try {
      // The solutions block is optional (e.g. attenuation-only configs omit
      // it); leave the default (has_source == false) when absent.
...
public:
  ...
  specfem::attenuation::Setup get_attenuation_setup() const {
    return config.get_attenuation_setup();
  }
