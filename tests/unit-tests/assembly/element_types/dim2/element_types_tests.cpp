...
using ElementTypes2D =
    specfem::assembly::element_types<specfem::element::dimension_tag::dim2>;

// Test 1: Homogeneous mesh - all elements elastic_psv/isotropic/none/none
TEST(ElementTypes2D, HomogeneousMesh) {
  const int nspec = 4;
  const int ngllz = 5;
  const int ngllx = 5;
  ...
  for (int i = 0; i < nspec; i++) {
    medium_tags(i) = specfem::element::medium_tag::elastic_psv;
  }
  ...
  ElementTypes2D element_types(nspec, ngllz, ngllx, medium_tags, property_tags,
  ...
  // Test host view
  auto elastic_psv_host = element_types.get_elements_on_host(
      specfem::element::medium_tag::elastic_psv);
  EXPECT_EQ(elastic_psv_host.extent(0), 4);
  EXPECT_EQ(elastic_psv_host(0), 0);
  EXPECT_EQ(elastic_psv_host(1), 1);
  EXPECT_EQ(elastic_psv_host(2), 2);
  EXPECT_EQ(elastic_psv_host(3), 3);
...
// Test 2: Mixed media - elastic_psv and acoustic
TEST(ElementTypes2D, MixedMedia) {
  const int nspec = 6;
  const int ngllz = 5;
  const int ngllx = 5;
  ...
  auto elastic_psv_host = element_types.get_elements_on_host(
      specfem::element::medium_tag::elastic_psv);
  EXPECT_EQ(elastic_psv_host.extent(0), 3);
  EXPECT_EQ(elastic_psv_host(0), 0);
  EXPECT_EQ(elastic_psv_host(1), 1);
  EXPECT_EQ(elastic_psv_host(2), 2);
  ...
  // Test device views
  auto elastic_psv_device = element_types.get_elements_on_device(
      specfem::element::medium_tag::elastic_psv);
  EXPECT_EQ(elastic_psv_device.extent(0), 3);
...
// Test 3: Medium + property combinations
TEST(ElementTypes2D, MediumAndProperty) {
  const int nspec = 3;
  const int ngllz = 5;
  const int ngllx = 5;
  ...
  auto iso_host = element_types.get_elements_on_host(
      specfem::element::medium_tag::elastic_psv,
      specfem::element::property_tag::isotropic);
  EXPECT_EQ(iso_host.extent(0), 3);
  EXPECT_EQ(iso_host(0), 0);
  EXPECT_EQ(iso_host(1), 1);
  EXPECT_EQ(iso_host(2), 2);

  auto iso_device = element_types.get_elements_on_device(
      specfem::element::medium_tag::elastic_psv,
      specfem::element::property_tag::isotropic);
  EXPECT_EQ(iso_device.extent(0), 3);
}

// Test 4: Medium + property + attenuation
TEST(ElementTypes2D, MediumPropertyAttenuation) {
  const int nspec = 4;
  const int ngllz = 5;
  const int ngllx = 5;
  ...
  // Test 2-arg query aggregates across attenuation
  EXPECT_EQ(element_types.get_number_of_elements(
                specfem::element::medium_tag::elastic_psv,
                specfem::element::property_tag::isotropic),
            4);
...
// Test 5: Medium + property + boundary
TEST(ElementTypes2D, MediumPropertyBoundary) {
  const int nspec = 6;
  const int ngllz = 5;
  const int ngllx = 5;
  ...
  Kokkos::View<specfem::element::boundary_tag *,
      ...
      boundary_tags("boundary_tags", nspec);
  for (int i = 0; i < 3; i++) {
    boundary_tags(i) = specfem::element::boundary_tag::acoustic_free_surface;
  }
  for (int i = 3; i < 6; i++) {
    boundary_tags(i) = specfem::element::boundary_tag::none;
  }
  ...
  auto free_surface_host = element_types.get_elements_on_host(
      specfem::element::medium_tag::acoustic,
      specfem::element::property_tag::isotropic,
      specfem::element::boundary_tag::acoustic_free_surface);
  EXPECT_EQ(free_surface_host.extent(0), 3);
  EXPECT_EQ(free_surface_host(0), 0);
  EXPECT_EQ(free_surface_host(1), 1);
  EXPECT_EQ(free_surface_host(2), 2);

  auto none_boundary_device = element_types.get_elements_on_device(
      specfem::element::medium_tag::acoustic,
      specfem::element::property_tag::isotropic,
      specfem::element::boundary_tag::none);
  EXPECT_EQ(none_boundary_device.extent(0), 3);
}

// Test 6: Empty mesh (nspec=0)
TEST(ElementTypes2D, EmptyMesh) {
  const int nspec = 0;
  const int ngllz = 5;
  const int ngllx = 5;
  ...
  auto host_view = element_types.get_elements_on_host(
      specfem::element::medium_tag::elastic_psv);
