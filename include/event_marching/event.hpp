...
public:
  ...
  event(specfem::event_marching::precedence p):
    precedence(p){}
  ...
  virtual int call() {
    return specfem::event_marching::DEFAULT_EVENT_CALL();
  }
...
public:
  ...
  arbitrary_call_event(std::function<int()> event_call_func, specfem::event_marching::precedence p):
    event_call_func(event_call_func),
    event(p){}
  
  int call() {
    return event_call_func();
  }
private:
