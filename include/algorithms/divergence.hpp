...
KOKKOS_FUNCTION void divergence(
  ...
  Kokkos::parallel_for(
      ...
      [=](const int &ixz) {
        ...
        type_real temp1l[components * ThreadTile * ThreadTile] = { 0.0 };
        type_real temp2l[components * ThreadTile * ThreadTile] = { 0.0 };
          ...
          for (int l = 0; l < NGLL; ++l) {
            for (int Tid = 0; Tid < ThreadTile; ++Tid) {
              l_f[Tid] = (Tid + iz < NGLL)
                             ...
                             : 0.0;
              l_quad[Tid] = (Tid + ix < NGLL) ? hprimewgll(ix + Tid, l) : 0.0;
            }

            for (int Tiz = 0; Tiz < ThreadTile; ++Tiz) {
              for (int Tix = 0; Tix < ThreadTile; ++Tix) {
                temp1l[icomponent * ThreadTile * ThreadTile + Tiz * ThreadTile +
            ...
            for (int Tid = 0; Tid < ThreadTile; ++Tid) {
              l_f[Tid] = (Tid + ix < NGLL)
                             ...
                             : 0.0;
              l_quad[Tid] = (Tid + iz < NGLL) ? hprimewgll(iz + Tid, l) : 0.0;
            }

            for (int Tiz = 0; Tiz < ThreadTile; ++Tiz) {
              for (int Tix = 0; Tix < ThreadTile; ++Tix) {
                temp2l[icomponent * ThreadTile * ThreadTile + Tix * ThreadTile +
