...
void construct_demo_mesh(specfem::mesh::mesh & mesh, const specfem::quadrature::quadratures & quad,
    const int nelemx, const int nelemz, std::vector<specfem::adjacency_graph::adjacency_pointer>& removals);
...
struct simulation_params{
   ...
   */
  simulation_params() : _t0(0), _dt(1), _tmax(0), _nsteps(0), _simulation_type(specfem::simulation::type::forward),
      _mesh(specfem::mesh::mesh()), _quadratures(_default_quadrature()),
      _sources(std::vector< std::shared_ptr<specfem::sources::source> >()),
      _receivers(std::vector< std::shared_ptr<specfem::receivers::receiver> >()),
      _seismogram_types(std::vector<specfem::enums::seismogram::type>()),
      _nseismogram_steps(0),
  ...
  simulation_params& add_source(std::shared_ptr<specfem::sources::source> source){
    _sources.push_back(source);
    return *this;
  }
  simulation_params& add_receiver(std::shared_ptr<specfem::receivers::receiver> receiver){
    _receivers.push_back(receiver);
    return *this;
  }
  simulation_params& add_seismogram_type(specfem::enums::seismogram::type type){
  ...
  simulation_params& use_demo_mesh(std::vector<specfem::adjacency_graph::adjacency_pointer>& removals){
  ...
  specfem::compute::assembly build_assembly(){
    specfem::compute::assembly assembly(_mesh, _quadratures, _sources, _receivers,
            _seismogram_types, _t0, _dt, _nsteps, _nseismogram_steps, _simulation_type);
    return assembly;
  }
 private:
  ...
  void _update_timevars(int set_ind){
    int8_t set_prio_prev = (set_ind == 0)? _t0_adj_prio:
                          ((set_ind == 1)? _dt_adj_prio:
                          ((set_ind == 2)? _tmax_adj_prio:
                          _nstep_adj_prio));
    if(set_prio_prev > _t0_adj_prio){_t0_adj_prio++;}else if(set_prio_prev == _t0_adj_prio){_t0_adj_prio = 0;}
    if(_t0_adj_prio == 3) _t0 = _tmax - _nsteps * _dt;
    if(set_prio_prev > _dt_adj_prio){_dt_adj_prio++;}else if(set_prio_prev == _dt_adj_prio){_dt_adj_prio = 0;}
    if(_dt_adj_prio == 3) _dt = (_tmax - _t0) / _nsteps;
    if(set_prio_prev > _tmax_adj_prio){_tmax_adj_prio++;}else if(set_prio_prev == _tmax_adj_prio){_tmax_adj_prio = 0;}
    if(_tmax_adj_prio == 3) _tmax = _t0 + _nsteps * _dt;
    if(set_prio_prev > _nstep_adj_prio){_nstep_adj_prio++;}else if(set_prio_prev == _nstep_adj_prio){_nstep_adj_prio = 0;}
    if(_nstep_adj_prio == 3) _nsteps = std::ceil((_tmax - _t0)/_dt);
  ...
  int _nseismogram_steps; //TODO this is max_sig_step; verify that this actually is nseismo_steps
...
void print_index_mappings(specfem::compute::simulation_field<WavefieldType> field){
  std::cout << "[";
  for(int ispec = 0; ispec < field.nspec; ispec++){
    std::cout << "[";
    for(int iz = 0; iz < field.ngllz; iz++){
      std::cout << "[";
      for(int ix = 0; ix < field.ngllx; ix++){
        std::cout << field.h_index_mapping(ispec,iz,ix) << ",";
      }
      std::cout << "],";
    }
    std::cout << "],";
...
template <int dim>
void print_point_locations(specfem::compute::assembly assembly){
  const auto & pts = assembly.mesh.points.coord;
  int nspec = assembly.mesh.nspec;
  int ngllx = assembly.mesh.ngllx;
  int ngllz = assembly.mesh.ngllz;
    ...
    for(int iz = 0; iz < ngllz; iz++){
      ...
      for(int ix = 0; ix < ngllx; ix++){
        ...
        for(int id = 0; id < dim; id++){
          std::snprintf(tmp,30,"%14.10f,",pts(id,ispec,iz,ix));
          std::cout << tmp;
        }
        std::cout << "],";
      }
