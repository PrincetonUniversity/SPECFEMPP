...
In this example we simulate wave propagation through a 2-dimensional homogeneous medium. The goal of this example is to
show fundamental differences between seismogram generated by single and multiple sources i.e., interference patterns.
...
Note the path of the database file generated after successfully running the
mesher. Generally, the database will be located within ``OUTPUT_FILES``
directory within root of ``SPECFEM2D``.
